...
- **Slice 4: Tawk as the second DOM-drive vendor - BUILT.** (Crisp was dropped: it is API-send like Gorgias and only ~145 stores, so it would not exercise WidgetDriver.) Tawk is a `VendorConfig` over WidgetDriver + a new frame-by-content-marker capability; verified live with the shipped code up to the composer on standard widgets. Outstanding: (a) one real HITL send to confirm Enter transmits + onChatMessageVisitor confirms; (b) the "help center" widget variant (mohifashion) reaches the frame+entry but not the composer; (c) reply-capture for tawk (add_init_script visitor-email injection) is the project-wide open question, deliberately deferred.
