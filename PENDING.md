...
- **Real online sends owed (verify-to-composer done, not send-proven):** LiveChat, Chatra, HelpScout, and Intercom. Each needs one real send against an ONLINE store, with a human in the loop, to prove transmit + confirm (like Tawk's allurepack send). HelpScout additionally needs its send path built (the "Ask" form is fill-email + Send + a thank-you-state confirm, not type+Enter+dom_echo). Intercom needs its `startConversation` transmit confirmed.
- **Zendesk (654, biggest) - DEFERRED, needs a focused session.** Unreliable headless: ~half the stores never initialise `zE`; the DOM composer surfaces only intermittently; modern messaging send is an async newConversation->sendMessage flow. Decide DOM-drive (on stores that render a composer) vs api-send. See `research/vendor-landscape.md`.
