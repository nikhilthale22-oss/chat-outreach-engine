...
- **Next vendor (breadth).** Pick another DOM-drive vendor from the universe (LiveChat, Olark, Chatra, Re:amaze, Smartsupp, JivoChat, Intercom-prefill) and add it as a config. Iframe support + dom_echo now exist, so most should be config-only.
